...
// @var title Infinite City
// @var author TekF
// @var url https://www.shadertoy.com/view/4df3DS
...
vec3 getSceneColor( vec3 rayStart, vec3 rayDir )
{
	...
	//	rayStart += -10.0*vec3(sin(rot.x),0,cos(rot.x));
		...
		vec2 rot = vec2(-.2,.28)+vec2(1.6,.5)*mouse;
		 ...
		 rayStart = vec3(0,5,0) + vec3(1,0,1)*iGlobalTime + vec3(0,6,0)*mouse.y;
		rayStart += -10.0*vec3(sin(rot.x),0,cos(rot.x));
...
/*	float f = Voronoi(gl_FragCoord.xy/40.0).z;
    ...
    vec3 result = getSceneColor( rayStart, rayDir );
