...
// @var title Torus Journey
// @var author PauloFalcao
// @var url https://www.shadertoy.com/view/MdlGzn

// @var eyePos 31.28 -1.97 -7.14
...
vec3 getSceneColor(vec3 prp, vec3 scp)
{
  //Raymarching
  ...
  if (f<maxd){
    ...
    if (s.y==2.0){
      ...
      if (f<maxd){
        ...
        return vec3((objColor*0.8+objColor2*0.2)*(1.0-f0*.03));
      } else return vec3(objColor*0.8*(1.0-f0*.03));

    } else return vec3(objColor*(1.0-f0*.03));
...
void main(void){
  ...
  vec3 col = getSceneColor(prp, scp);
