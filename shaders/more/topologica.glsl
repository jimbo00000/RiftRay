...
// @var title Topologica
// @var author otaviogood
// @var url https://www.shadertoy.com/view/4djXzz
...
{
    vec2 fr = fract(uv);
    vec2 smoo = fr*fr*(3.0-2.0*fr);
    vec2 fl = floor(uv);
    vec2 uv2 = smoo + fl;
    return texture2D(iChannel0, (uv2 + 0.5)/iChannelResolution[0].xy).y;
}
...
vec3 getSceneColor(in vec3 camPos, in vec3 relVec)
{
	float t = 0.0;
	float inc = 0.02;
	float maxDepth = 70.0;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	float mx=0.;//iMouse.x/iResolution.x*PI*2.0 + iGlobalTime * 0.01;
	float my=0.;//-iMouse.y/iResolution.y*10.0 + sin(iGlobalTime * 0.03)*0.2+0.2;//*PI/2.01;
	...
	vec3 finalColor = getSceneColor(camPos, relVec);
