...
// @var title Marching Cubes
// @var author fizzer
// @url https://www.shadertoy.com/view/MdsGz8
...
vec3 getSceneColor( in vec3 ro, in vec3 rd )
{
	...
	for(int i = 0; i < 200; i += 1)
	{
		...
		if(abs(d) < 1e-3)
		{
			col.rgb = veil(ro) * mix(vec3(0.1, 0.1, 0.4) * 0.25, vec3(1.0),
...
void main(void)
{
    ...
    gl_FragColor.rgb = getSceneColor( ro, rd );
