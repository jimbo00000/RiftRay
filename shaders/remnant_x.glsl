...
// @var url https://www.shadertoy.com/view/4sjSW1

// @var eyePos 2.039 -0.893 1.984
// @var eyePos2 1.024 -1.036 3.348
...
{
	float t = .05 + 0.05 * texture2D(iChannel0, gl_FragCoord.xy / iChannelResolution[0].xy).y;
	vec3 p = vec3(0.0);
    float oldT = 0.0;
    bool hit = false;
    float glow = 0.0;
...
vec3 getSceneColor( vec3 cameraPos, vec3 dir )
{
	gTime = (iGlobalTime)*.01 + 15.00;

/*
    //cameraPos	= CameraPath(gTime);
