...
vec3 getSceneColor( in vec3 cp, in vec3 d )
{
    Hit h;
	vec3 col = vec3(0.16);
	vec3 ray = cp.zyx * vec3(-1.,1.,1.); // probably breaks Shadertoy
    vec3 dir = d.zyx * vec3(-1.,1.,1.);
	float dist = 0.0;
