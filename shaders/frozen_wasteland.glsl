...
// @var title Frozen wasteland
// @var url https://www.shadertoy.com/view/Xls3D2
...
vec3 getSceneColor( in vec3 ro, in vec3 rd )
{
    ...
    float fg;
	float rz = march(ro,rd, fg, vec2(0.));//fragCoord);
    ...
    if ( rz < FAR )
    {
        ...
        // Fresnel doesn't seem to make much difference. removed....
        //float fre = pow( clamp(1.+dot(ref, rd),0.0,1.0), 150. );
        col = vec3(.7);
